...
namespace NatanielSoaresRodrigues.ProjectCustomGame.Views
{
	public class MainView : MonoBehaviour {
		...
		public DialogController dialoguesController{ get; private set; }
		public CharactersController charactersController{ get; private set; }
		public ScenesController scenesController { get; private set;}
		...
		void Start () {
			...
			dialoguesController = new DialogController (c_showDialogResponse);
			dialoguesController.initialize ();
		...
		void c_showDialogResponse(object sender, DialogEventArgs e){
			...
			if (e.Dialogues.Length == 1) {
				...
				selectChar (dialoguesController.CurrentDialog.IdCharacter);
				selectScene (dialoguesController.CurrentDialog.IdScene);
		...
		void selectScene(string idScene)
		{
			scenesController.selectCurrent (idScene);
		}
		...
		public void optionSelected(int option)
		{
			if (!canSelect)
				return;
			try {
				dialoguesController.selectOption (option);
			} catch (Exception ex) {
		...
		void checkDialog()
		{
			...
			try {
				dialoguesController.checkDialog ();
			} catch (Exception ex) {
				Debug.Log (ex.Message);
			}
		}
